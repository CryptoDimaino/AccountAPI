...
export class CodeListComponent implements OnInit {
  ...
  // API data
  private response: Response<CodeList[]>;
  ...
  async ngOnInit() {
     ...
     if(this.response.DidError) {
       ...
       // Select
       this.page = 1;
       this.numberOfCodes = this.codeList.length;
  ...
  async loadResponse() {
    await this.codeService.getCodes().then((data: Response<CodeList[]>) => {
      this.response = data;
      this.codeList = this.response.Model;
      this.loaded = true;
    })
  }
